...
frame_height = 150
frame = pygame.imageload(f'Assets/bg3.jpg')
...
dodgy = Message(WIDTH // 2, HEIGHT // 3, 80, "Bouncing", title_font, WHITE, win)
walls = Message(WIDTH // 2, HEIGHT // 2, 60, "Balls", title_font, WHITE, win)
...
# Define a variable to control the bounce when the space bar is pressed
bounce_on_space = False
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_ESCAPE or event.key == pygame.K_q:
                running = False
            if event.key == pygame.K_SPACE and game_page:
                # Set the variable to control the bounce
                bounce_on_space = True
    ...
    if game_page:
        ...
        p.update(player_alive, bounce_on_space)  # Use the variable for bounce control
